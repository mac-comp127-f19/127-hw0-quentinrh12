...
public class Emojis {
    ...
    public static GraphicsGroup createSmileyFace(double size) {
        ...
        Ellipse eyeone = createEye(size);
        eyeone.setCenter(size * .333, size * 0.333);
        group.add(eyeone);

        Ellipse eyetwo = createEye(size);
        eyetwo.setCenter(size * .666, size * 0.333);
        group.add(eyetwo);
    ...
    private static Ellipse createEye(double size) {
        double diameter = size * 0.1;
        Ellipse eye = new Ellipse(0,0, diameter, diameter);
        eye.setFillColor(new Color(0xFC000000, true));
        return eye;
    }
}
